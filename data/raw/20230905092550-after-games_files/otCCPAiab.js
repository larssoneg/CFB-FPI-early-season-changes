 ...
 */
 (function () {
	...
	window.otCcpaOptOut = (function () {
		...
		function init() {
			...
			function checkIfActiveGroupsExist() {
				if (window.OneTrust && window.OnetrustActiveGroups && (activeGroupsAreLoaded || window.OnetrustActiveGroups !== ',,' || retryCount > 10)) {
					retryCount = 0;
					groupsUpdated();
				} else {
					retryCount++;
			...
			if (isLocationApplicable()) {
				...
				function groupsUpdated() {
					...
					if (consent && consent.USPrivacy && consent.consentedDate) {
						...
						var activeGroupsArr = !window.OnetrustActiveGroups.slice(1, -1) ? [] : window.OnetrustActiveGroups.slice(1, -1).split(",");
			...
			function isLocationApplicable() {
				...
				var locationData = window.OneTrust.geolocationResponse,
				...
				if (locationSetting === 'all') {
					...
					applicable = (locationData.countryCode.toString().toLowerCase() === 'us');
				} else if (locationSetting === 'ca') {
					applicable = (locationData.stateCode.toString().toLowerCase() === 'ca');
				} else {
					applicable = false;
				}
	...
	window.addEventListener('OneTrustGroupsUpdated', function () {
		...
		window.otCcpaOptOut.init();
	});
})();

}
/*
     FILE ARCHIVED ON 12:04:22 Sep 05, 2023 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 00:22:11 Sep 08, 2023.
...
/*
playback timings (ms):
  captures_list: 1990.027
  exclusion.robots: 0.065
  exclusion.robots.policy: 0.057
  cdx.remote: 0.054
  esindex: 0.009
  LoadShardBlock: 1696.618 (3)
  PetaboxLoader3.resolve: 2645.026 (3)
  PetaboxLoader3.datanode: 638.469 (5)
  load_resource: 1896.594 (2)
*/